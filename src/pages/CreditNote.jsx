...
const CreditNote = () => {
    ...
    // 🖨️ CETAK LAPORAN REKAP CREDIT NOTE (A4 LANDSCAPE POPUP)
    const handlePrintReport = async () => {
        if (!data || data.length === 0) {
            Swal.fire({
                title: 'DATA KOSONG',
                text: 'Tidak ada data Credit Note yang dapat dicetak.',
                icon: 'warning',
        ...
        const rowsHtml = data.map((item, idx) => {
            const nilai = Number(item.artcnh_total || 0);
            totalNilaiAll += nilai;
            const isPosted = item.artcnh_postingyn === 'Y';

            return `
                <tr style="font-family: monospace; font-size: 10px;">
                    <td style="border: 1px solid #333; padding: 5px; text-align: center;">${idx + 1}</td>
                    <td style="border: 1px solid #333; padding: 5px; font-weight: bold; color: #0284c7;">${item.artcnh_no || '-'}</td>
                    <td style="border: 1px solid #333; padding: 5px; text-align: center;">${String(item.artcnh_tanggal || '').split('T')[0] || '-'}</td>
                    ...
                    <td style="border: 1px solid #333; padding: 5px; font-family: sans-serif;">${item.artcnh_keterangan || '-'}</td>
                    <td style="border: 1px solid #333; padding: 5px; text-align: right; font-weight: bold;">Rp ${nilai.toLocaleString('id-ID')}</td>
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Laporan Rekap Credit Note - ${namaCabang}</title>
                <style>
                    @page { 
                    ...
                    <table style="width: 100%; border: none;">
                        <tr>
                            ...
                            <td style="width: 45%; text-align: right; vertical-align: middle; border: none;">
                                <div style="font-size: 14px; font-weight: 900; text-decoration: underline; letter-spacing: 0.5px;">LAPORAN REKAPITULASI CREDIT NOTE</div>
                ...
                <table>
                    <thead>
                        <tr style="background-color: #cbd5e1; font-weight: bold; font-size: 10px; text-align: center;">
                            ...
                            <th style="border: 1px solid #475569; padding: 6px 5px; width: 14%;">NO. CREDIT NOTE</th>
                            <th style="border: 1px solid #475569; padding: 6px 4px; width: 9%;">TANGGAL</th>
                            <th style="border: 1px solid #475569; padding: 6px 5px; width: 22%;">CUSTOMER</th>
                            <th style="border: 1px solid #475569; padding: 6px 4px; width: 12%;">ALASAN</th>
                            <th style="border: 1px solid #475569; padding: 6px 5px; width: 18%;">KETERANGAN</th>
                            <th style="border: 1px solid #475569; padding: 6px 5px; width: 13%; text-align: right;">TOTAL NILAI</th>
                            <th style="border: 1px solid #475569; padding: 6px 4px; width: 8%;">STATUS</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${rowsHtml}
                    ...
                    <tfoot>
                        <tr style="background-color: #f1f5f9; font-weight: bold; font-size: 10px; font-family: monospace;">
                            <td colspan="6" style="border: 1px solid #475569; padding: 6px; text-align: center;">TOTAL (${data.length} DOKUMEN) :</td>
                            <td style="border: 1px solid #475569; padding: 6px; text-align: right; font-weight: 900; color: #e11d48;">Rp ${totalNilaiAll.toLocaleString('id-ID')}</td>
