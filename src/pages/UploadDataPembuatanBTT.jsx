...
import { FileSpreadsheet, UploadCloud, Info, CheckCircle2, RefreshCw, X, Building, Layers } from 'lucide-react';
import api from '../api/axios';
import Swal from 'sweetalert2';

export default function UploadDataPembuatanBTT({ isDarkMode = false }) {
    // Modal SOP
    ...
    const [tipe, setTipe] = useState('1'); // "1" = Luar Kota, "2" = Dalam Kota
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                const res = await api.get(`/marketing/upload-btt/suggest-customer?q=${encodeURIComponent(queryCust)}`, {
    ...
    const handleParseCSV = async (e) => {
        ...
        try {
            ...
            const res = await api.post('/marketing/upload-btt/parse-csv', formData, {
                headers: {
    ...
    const handleSaveBatch = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await api.post('/marketing/upload-btt/save-batch', {
            ...
            Swal.fire({
                ...
                text: res.data?.message || 'Data BTT siap diproses ke operasional.',
        ...
        <div className={`p-6 min-h-screen space-y-6 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-[#f8fafc] text-slate-800'}`}>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400">Import CSV kargo korporat (Kimia Farma, Merck) 37/38 kolom</p>
                    </div>
                </div>
                ...
                <button
                    ...
                    className="px-4 h-10 border border-emerald-300 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl text-xs font-bold flex items-center gap-2 cursor-pointer transition"
                >
                    <Info size={16} /> Lihat SOP Upload
                </button>
            </div>

            {/* Modal SOP Bawaan Sistem */}
            {showSOP && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl border border-slate-100 p-6 animate-in zoom-in-95 duration-200 space-y-4">
                        <div className="text-center pb-2 border-b border-slate-100">
                            <h2 className="text-base font-black text-emerald-600 uppercase">SOP Upload CSV Kimia Farma / Merck</h2>
                        </div>
                        <ol className="text-xs text-slate-600 space-y-2 list-decimal list-inside leading-relaxed font-medium">
                            <li>Pilih kustomer (Kimia Farma / Merck) terlebih dahulu.</li>
                            <li>Tentukan delimiter yang digunakan pada berkas CSV (Koma atau Titik Koma)[cite: 12].</li>
                            <li>Pilih tipe pengiriman yang tersedia: <b>LUAR KOTA (37 Kolom)</b> atau <b>DALAM KOTA (38 Kolom)</b>[cite: 12].</li>
                            <li>Pastikan kolom data sudah terisi sesuai template sebelum menekan tombol <b>Upload</b>[cite: 12].</li>
                        </ol>
                        <div className="p-3 bg-rose-50 border-l-4 border-rose-500 text-rose-700 text-[11px] leading-relaxed font-semibold">
                            * Catatan: Untuk mengurangi potensi kesalahan, pastikan format tanggal menggunakan MM/DD/YYYY dan nilai desimal menggunakan titik (.)[cite: 12].
                        </div>
                        <div className="flex justify-center pt-2">
                            <button
                                type="button"
                                onClick={() => setShowSOP(false)}
                                className="px-6 h-10 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs uppercase cursor-pointer"
                            >
                                Mengerti
                            </button>
                        </div>
            ...
            <form onSubmit={handleParseCSV} className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-xs font-bold">
                    ...
                    <div className="relative" ref={suggestRef}>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full h-11 pl-9 pr-3 border border-slate-200 rounded-xl uppercase outline-none focus:border-emerald-500"
                            />
                        </div>
                            ...
                            <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto z-50 divide-y divide-slate-100 text-xs">
                                {suggestList.map((c, idx) => (
                                    <div
                                        ...
                                        className="p-3 hover:bg-emerald-50 cursor-pointer font-bold text-slate-700"
                                    >
                    ...
                    <div>
                        <label className="text-slate-600 uppercase block mb-1.5">
                            Delimiter yang digunakan : <span className="text-rose-500">*</span>
                        </label>
                        <select
                            value={delimiter}
                            onChange={(e) => setDelimiter(e.target.value)}
                            className="w-full h-11 px-3 border border-slate-200 rounded-xl outline-none focus:border-emerald-500 bg-white text-slate-700"
                        >
                    ...
                    <div>
                        <label className="text-slate-600 uppercase block mb-1.5">
                            Tipe Pengiriman : <span className="text-rose-500">*</span>
                        </label>
                        <select
                            value={tipe}
                            ...
                            className="w-full h-11 px-3 border border-slate-200 rounded-xl outline-none focus:border-emerald-500 bg-white text-slate-700"
                        >
                            <option value="1">LUAR KOTA (37 Kolom)</option>
                            <option value="2">DALAM KOTA (38 Kolom)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        className="block w-full text-xs text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer border border-slate-200 rounded-xl p-2"
                    />
                </div>
                ...
                <div className="flex justify-end pt-2 border-t border-slate-100">
                    <button
                        ...
                        className="px-6 h-11 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-md text-xs flex items-center gap-2 cursor-pointer disabled:opacity-50 transition"
                    >
                        {loadingParse ? <RefreshCw size={14} className="animate-spin" /> : <UploadCloud size={14} />}
                        {loadingParse ? "Mengekstrak CSV..." : "Upload & Parse CSV"}
                    </button>
                </div>
            </form>

            {/* Tabel Review Hasil Parsing */}
            {parsedData.length > 0 && (
                <div className="p-6 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-4 animate-in fade-in duration-200">
                    <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                        <span className="text-xs font-bold uppercase text-emerald-700">
                            Pratinjau Data ({parsedData.length} Baris Dokumen Ditemukan)
                        </span>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            {savingBatch ? "Menyimpan..." : "Konfirmasi & Terbitkan BTT"}
                        </button>
                    </div>
                        ...
                        <table className="w-full text-xs text-left">
                            <thead className="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-600 uppercase sticky top-0">
                                <tr>
                                    ...
                                    {tipe === "2" && <th className="p-3 text-right">HARGA (RP)</th>}
                                    <th className="p-3">NO. KENDARAAN</th>
                                    <th className="p-3">NO. SKB</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {parsedData.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50">
                                        <td className="p-3 font-mono font-bold text-indigo-600">{row.no_dn}</td>
                                        ...
                                        <td className="p-3 font-mono">{row.no_kendaraan || "-"}</td>
                                        <td className="p-3 font-mono">{row.no_skb || "-"}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
