...
import { useDarkMode } from '../../context/DarkModeContext';
  ...
  const fetchProfile = async () => {
    try {
      ...
      const userData = res.data; // Data dari backend
      
      setUser({
        name: userData.realname || 'User',
        email: userData.username ? `${userData.username}@dakota1.com` : 'user@dakota.com',
        // Ambil role dari userData.usertype sesuai log console
        role: userData.usertype || '', 
        division: userData.division || '',
        // Ambil foto profil dengan pengecekan field
        profileimage: userData.profileimage || userData.profile_image 
      });

      console.log("Role User Berhasil Di-set:", userData.usertype);
    } catch (err) {
        ...
        <NavLink to="/account" className="no-underline">
          {({ isActive }) => (
            ...
            >
              ...
              >
               {(localStorage.getItem('profile_image') || user.profileimage) ? (
                  <img src={user.profileimage ? user.profileimage : '/default-avatar.png'} alt="profile" className="w-full h-full object-cover" onError={(e) => { e.target.src = ""; }} />
                ) : (
